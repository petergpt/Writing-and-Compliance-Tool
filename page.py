  ...
  if st.button('Check Compliance', key='button2'):
    if st.session_state["guidance_input"]:
      with st.spinner('Checking for compliance...'):
        ...
        static_part = "Here is the guidance: " + st.session_state[
            "guidance_input"]
        ...
        messages = [
            ...
            },
            {
                ...
                "user",
                "content":
                f"Analyse the following text: {st.session_state['generated_text']}"
            },
        ]
        response = openai_utils.send_request_to_openai(messages)
        st.session_state["result"] = response['choices'][0]['message'][
            'content']  # Store the content in the session state

    else:
      st.warning(
          'Please enter guidance and ensure text is generated in Section 1')
...
def test_persona_perception():
  ...
  persona_option = st.selectbox('Select a persona',
                                list(PERSONAS_OPTIONS.keys()),
                                key='persona')
  ...
  if st.button('Test Persona', key='button3'):
    if st.session_state["questions_input"] and st.session_state["persona_input"]:
      with st.spinner('Testing persona perception...'):
        ...
        messages = [
            ...
            },
            {
                ...
                "user",
                "content":
                f"Read the following text: {st.session_state['generated_text']}"
            },
        ]
        response = openai_utils.send_request_to_openai(messages)
      ...
      st.warning(
          'Please enter questions and persona, and ensure text is generated and guidance is entered'
      )
